...
const Profile = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      const response = await axios.post('https://36rwrd.online/api/v1/user/avatar', formData, {
        headers: {
  ...
  const handleProfileUpdate = async () => {
    ...
    try {
      const response = await axios.put(
        'https://36rwrd.online/api/v1/user/profile',
        formData,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleError = (error: any) => {
    if (error.response) {
      ...
      toast.error(serverError.message || 'حدث خطأ أثناء التحديث');
        ...
        <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
          <div className="relative z-30 mx-auto -mt-22 h-30 w-full max-w-30 rounded-full bg-white/20 p-1 backdrop-blur sm:h-44 sm:max-w-44 sm:p-3">
            <div className="relative drop-shadow-2 h-full w-full">
              <span className="h-full w-full rounded-full flex items-center justify-center bg-blue-600 text-white text-lg font-semibold shadow-md">
                {avatarUrl ? (
                  <img
                    src={avatarUrl}
                    alt="avatar"
          ...
          <div className="mt-4">
            <h3 className="mb-1.5 text-2xl font-semibold text-black dark:text-white">
              {user ? user.name : 'تحميل...'}
            </h3>
            <p className="font-medium">{user ? user.role : '...'}</p>

            <div className="mx-auto max-w-180 mt-4">
              <h4 className="font-semibold text-black dark:text-white">حول</h4>
              {isEditing ? (
                <textarea
                  name="bio"
                  value={formData.bio}
                  onChange={handleInputChange}
                  className="mt-4.5 w-full rounded-md border border-stroke p-2 dark:border-strokedark dark:bg-boxdark"
                  rows={4}
                  placeholder="اكتب نبذة عن نفسك..."
                />
              ) : (
                <p className="mt-4.5">{user?.bio || 'لا توجد نبذة تعريفية بعد'}</p>
              )}
            </div>
              ...
              {isEditing ? (
                <div className="grid gap-4">
                  <input
                    ...
                    className="rounded-md border border-stroke p-2 dark:border-strokedark dark:bg-boxdark"
                  />
                  <input
                    type="url"
                    name="twitter"
                ...
                <div className="flex items-center justify-center gap-3.5">
                  {user?.facebook && (
                    <Link to={user.facebook} className="hover:text-primary" aria-label="فيسبوك">
                  ...
                  {user?.twitter && (
                    <Link to={user.twitter} className="hover:text-primary" aria-label="تويتر">
                  ...
                  {user?.linkedin && (
                    <Link to={user.linkedin} className="hover:text-primary" aria-label="لينكدإن">
                      <svg className="fill-current" width="23" height="22" viewBox="0 0 23 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  {user?.instagram && (
                    <Link to={user.instagram} className="hover:text-primary" aria-label="إنستغرام">
                      <svg className="fill-current" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  {user?.github && (
                    <Link to={user.github} className="hover:text-primary" aria-label="جيت هب">
                      <svg className="fill-current" width="23" height="22" viewBox="0 0 23 22" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              {isEditing ? (
                <>
                  <button
                    ...
                    className="ml-2 rounded bg-primary py-2 px-4 text-white hover:bg-opacity-90"
                  >
                    حفظ التغييرات
                  </button>
                  <button
                    onClick={() => setIsEditing(false)}
                    className="rounded bg-gray-300 py-2 px-4 text-black hover:bg-gray-400"
                  >
                    إلغاء
                  </button>
                </>
