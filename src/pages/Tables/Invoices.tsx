...
const Invoices: React.FC = () => {
  ...
  const fetchInvoices = async () => {
    try {
      setLoading(true);
      const response = await fetch(`https://36rwrd.online/api/v1/invoices?page=${currentPage}&per_page=${itemsPerPage}`, {
        headers: {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setIsDeleting(true);
      const response = await fetch(`https://36rwrd.online/api/v1/invoices/${selectedInvoiceId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('access_token')}`,
        },
