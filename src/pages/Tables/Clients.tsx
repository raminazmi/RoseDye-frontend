...
const Clients: React.FC = () => {
  ...
  const fetchClients = async () => {
    try {
      ...
      setLoading(true);
      const response = await fetch(`https://36rwrd.online/api/v1/clients?page=${currentPage}&per_page=${itemsPerPage}`, {
        headers: {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://36rwrd.online/api/v1/clients/${selectedClientId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
