...
const OTPForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const code = otp.join('');
    try {
      const response = await fetch('https://36rwrd.online/api/v1/verify-otp', {
        method: 'POST',
      ...
      const subscriptionResponse = await fetch(`https://36rwrd.online/api/v1/subscriptions?client_id=${data.client_id}`, {
        headers: {
  ...
  const handleResendCode = async () => {
    ...
    try {
      const response = await fetch('https://36rwrd.online/api/v1/resend-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
